package com.match.web.controller.house;
...
import com.match.service.search.HouseBucketDTO;
...
public class HouseController {
    ...
    public String rentHousePage(@ModelAttribute RentSearch rentSearch,
        ...
        model.addAttribute("currentAreaBlock",RentValueBlock.matchArea(rentSearch.getPriceBlock()));
    ...
    public ApiResponse rentMapdistrict(@ModelAttribute MapSearchDistrict mapSearch){
        ...
        //通过小区名与城市名获取到房源数据
        result = houseService.districtMapQuery(mapSearch);
    ...
    public ApiResponse rentMapDistrict(@ModelAttribute SearchDistrict searchDistrict){
        ...
        ServiceMultiResult<HouseBucketDTO> result = searchService.mapAggregateDistrict(searchDistrict);
        List<HouseBuketRegionDTO> res = new ArrayList<>();
        result.getResult().forEach(dto ->{
            res.add(new HouseBuketRegionDTO(dto.getKey(),houseService.findRegion(dto.getKey()).getMessage(), dto.getCount())); ;
    ...
    public void  update(){
        ...
        all.forEach(house -> {
            if (house.getStatus() == 1){//只为status == 1 的创建索引
