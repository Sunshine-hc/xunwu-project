...
import com.match.base.RentValueBlock;
import com.match.entity.House;
import com.match.entity.HouseDetail;
import com.match.entity.HouseTag;
import com.match.entity.SupportAddress;
import com.match.repository.HouseDetailRepository;
import com.match.repository.HouseRepository;
import com.match.repository.HouseTagRepository;
...
public class SearchServiceImpl implements ISearchService {
    ...
    private void createOrUpdateIndex(HouseIndexMessage message){
        ...
        String address = city.getCnName() + region.getEnName() + house.getStreet() + house.getDistrict() +detail.getDetailAddress();

        ServiceResult<BaiduMapLocation> location = addressService.getBaiduMapLocation(city.getEnName(),address);
    ...
    public ServiceMultiResult<Long> query(RentSearch rentSearch) {
        ...
        //城市
        boolQuery.filter(
                QueryBuilders.termQuery(HouseIndexKey.CITY_EN_NAME,
...
//        System.out.println("boolQuery: " + boolQuery.toString());
    ...
    public ServiceMultiResult<Long> mapQuery(String cityEnName, String orderBy, String orderDirection, int start, int size) {
        ...
        if (response.status() != RestStatus.OK){
            logger.warn("Search starus is not ok for " + searchRequestBuilder);
            return new ServiceMultiResult<>(0,houseIds);
        }
        for (SearchHit hit : response.getHits()) {
    ...
    @Override
    public ServiceMultiResult<HouseBucketDTO> mapAggregateDistrict(SearchDistrict searchDistrict) {
        BoolQueryBuilder boolQuery = QueryBuilders.boolQuery();
...
//                ));
        ...
        List<HouseBucketDTO> buketDTOS = new ArrayList<>();
        if (response.status() != RestStatus.OK){
            logger.warn("Aggregate status is not ok for" + requestBuilder);
        ...
        Terms terms = response.getAggregations().get(HouseIndexKey.AGG_DISTRICT);
        for (Terms.Bucket bucket : terms.getBuckets()) {
            buketDTOS.add(new HouseBucketDTO(bucket.getKeyAsString(),bucket.getDocCount()));
        }
